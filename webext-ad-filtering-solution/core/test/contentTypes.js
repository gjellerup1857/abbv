...
const {contentTypes, RESOURCE_TYPES, SPECIAL_TYPES, ALLOWING_TYPES,
       enumerateTypes} = require(LIB_FOLDER + "/contentTypes");
...
describe("contentTypes.WEBBUNDLE", function() {
  it("should be numerical", function() {
    assertNumerical(contentTypes.WEBBUNDLE);
  });

  it("should be a positive non-fractional power of 2", function() {
    assertPositivePowerOfTwo(contentTypes.WEBBUNDLE);
  });

  it("should be less than or equal to 2 ** 30", function() {
    assertLessThanEqualTwoPowThirty(contentTypes.WEBBUNDLE);
  });
});
...
describe("*enumerateTypes()", function() {
  function testEnumerateTypes(types, expectedTypes, {selection} = {}) {
    ...
    for (let value of enumerateTypes(contentType, selection)) {
      ...
      assert.equal(actualContentType & value, 0, `${value} already yielded once`);

      actualContentType |= value;
    }

    assert.equal(actualContentType, expectedContentType, `${actualContentType} is not equal to ${expectedContentType}`);
  }
  ...
  it("should yield SCRIPT,IMAGE,STYLESHEET for SCRIPT,IMAGE,STYLESHEET and selection RESOURCE_TYPES", function() {
    testEnumerateTypes("SCRIPT,IMAGE,STYLESHEET", "SCRIPT,IMAGE,STYLESHEET", {selection: RESOURCE_TYPES});
  });

  it("should yield nothing for SCRIPT,IMAGE,STYLESHEET and selection SPECIAL_TYPES", function() {
    testEnumerateTypes("SCRIPT,IMAGE,STYLESHEET", "", {selection: SPECIAL_TYPES});
  });
  ...
  it("should yield nothing for CSP,DOCUMENT,GENERICHIDE and selection RESOURCE_TYPES", function() {
    testEnumerateTypes("CSP,DOCUMENT,GENERICHIDE", "", {selection: RESOURCE_TYPES});
  });
