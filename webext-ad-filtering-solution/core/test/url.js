...
const publicSuffixes = {...icannPublicSuffixes, ...nonIcannPublicSuffixes};
const {parseURL, isLocalhost, domainSuffixes, URLRequest, getBaseDomain,
       parseDomains, isValidHostname, matchesWildcards} =
  require(LIB_FOLDER + "/url");
...
describe("URLRequest", function() {
  describe("#get thirdParty()", function() {
    ...
    it("should return false for http://foo.com/ and foo.com", function() {
      testThirdParty("http://foo.com/", "foo.com", false, "same domain with TLD (.com) isn't third-party");
    });

    it("should return true for http://foo.com/ and bar.com", function() {
      testThirdParty("http://foo.com/", "bar.com", true, "same TLD (.com) but different domain is third-party");
    });

    it("should return false for http://foo.com/ and www.foo.com", function() {
      testThirdParty("http://foo.com/", "www.foo.com", false, "same domain but differend subdomain isn't third-party");
    });

    it("should return false for http://foo.example.com/ and bar.example.com", function() {
      testThirdParty("http://foo.example.com/", "bar.example.com", false, "same basedomain (example.com) isn't third-party");
    });

    it("should return true for http://foo.uk/ and bar.uk", function() {
      testThirdParty("http://foo.uk/", "bar.uk", true, "same TLD (.uk) but different domain is third-party");
    });

    it("should return true for http://foo.co.uk/ and bar.co.uk", function() {
      testThirdParty("http://foo.co.uk/", "bar.co.uk", true, "same TLD (.co.uk) but different domain is third-party");
    });

    it("should return false for http://foo.example.co.uk/ and bar.example.co.uk", function() {
      testThirdParty("http://foo.example.co.uk/", "bar.example.co.uk", false, "same basedomain (example.co.uk) isn't third-party");
    });

    it("should return false for http://1.2.3.4/ and 1.2.3.4", function() {
      testThirdParty("http://1.2.3.4/", "1.2.3.4", false, "same IPv4 address isn't third-party");
    });

    it("should return true for http://1.1.1.1/ and 2.1.1.1", function() {
      testThirdParty("http://1.1.1.1/", "2.1.1.1", true, "different IPv4 address is third-party");
    });

    it("should return false for http://[2001:db8:85a3::8a2e:370:7334]/ and [2001:db8:85a3::8a2e:370:7334]", function() {
      testThirdParty("http://[2001:db8:85a3::8a2e:370:7334]/", "[2001:db8:85a3::8a2e:370:7334]", false, "same IPv6 address isn't third-party");
    });

    it("should return true for http://[2001:db8:85a3::8a2e:370:7334]/ and [5001:db8:85a3::8a2e:370:7334]", function() {
      testThirdParty("http://[2001:db8:85a3::8a2e:370:7334]/", "[5001:db8:85a3::8a2e:370:7334]", true, "different IPv6 address is third-party");
    });

    it("should return false for http://example.com/ and example.com.", function() {
      testThirdParty("http://example.com/", "example.com.", false, "traling dots are ignored");
    });
  });
});
  ...
  it("should return correct values for all known suffixes", function() {
    ...
    for (let suffix in publicSuffixes) {
      ...
      for (let i = 0; i < offset + levels; i++) {
        ...
        assert.equal(getBaseDomain(hostname), expected, `getBaseDomain("${hostname}") == "${expected}"` +
...
describe("parseDomains()", function() {
  function testFilterDomainsParsing(source, separator, expected,
    ...
    if (repeatingDomains && source != "") {
      testFilterDomainsParsing(`${source}${separator}${source}`, separator, expected, {repeatingDomains: false});
    }
  }
...
describe("isValidHostname()", function() {
  ...
  it("should return true for example.*", function() {
    assert.strictEqual(isValidHostname("example.*"), true);
  });
  ...
  describe("wildcards", function() {
    it("should return true for asterisk wildcard", function() {
      assert.strictEqual(isValidHostname("example.*"), true, "Assertion failed for TLD");
      assert.strictEqual(isValidHostname("cdn.example.*"), true, "Assertion failed for domain with subdomain");
    });

    it("should return false for multiple asterisk wildcard", function() {
      assert.strictEqual(isValidHostname("example.**"), false, "Assertion failed for subsequent asterisks");
      assert.strictEqual(isValidHostname("*.example.*"), false, "Assertion failed for asterisks in the beginning and in the end");
      assert.strictEqual(isValidHostname("exa*mple.*"), false, "Assertion failed for asterisks in the beginning and in the middle");
    });

    it("should return false for multiple asterisk wildcard not preceeded with dot", function() {
      assert.strictEqual(isValidHostname("example*"), false);
    });

    it("should return false for asterisk wildcard in unexpected place", function() {
      assert.strictEqual(isValidHostname(".*example"), false, "Assertion failed for asterisk in the beginning");
      assert.strictEqual(isValidHostname("*example"), false, "Assertion failed for asterisk in the beginning and not preceeded with dot");
      assert.strictEqual(isValidHostname("exa.*mple"), false, "Assertion failed for asterisk in the middle");
      assert.strictEqual(isValidHostname("exa*mple"), false, "Assertion failed for asterisk in the middle and not preceeded with dot");
    });

    it("should return false for question mark wildcard", function() {
      assert.strictEqual(isValidHostname("example.?"), false, "Assertion failed for asterisk in the end");
      assert.strictEqual(isValidHostname("example?"), false, "Assertion failed for asterisk in the end and not preceeded with dot");
      assert.strictEqual(isValidHostname("exam?ple"), false, "Assertion failed for asterisk in the middle");
      assert.strictEqual(isValidHostname("?example"), false, "Assertion failed for asterisk in the beginning");
    });
  });
});

describe("matchesWildcards()", function() {
  it("should return true for regular match with TLDs", function() {
    assert.strictEqual(matchesWildcards("example.com", "example.*"), true);
  });

  it("should return true for regular match with ccSLD", function() {
    assert.strictEqual(matchesWildcards("example.co.uk", "example.*"), true, "Assertion failed for co.uk");
    assert.strictEqual(matchesWildcards("images.example.com.br", "example.*"), true, "Assertion failed for com.br");
  });

  it("should return true for regular match with TLD: foo", function() {
    assert.strictEqual(matchesWildcards("example.foo", "example.*"), true);
  });

  it("should return true for regular match with subdomains", function() {
    assert.strictEqual(matchesWildcards("images.example.com", "example.*"), true, "Assertion failed for third level subdomain");
    assert.strictEqual(matchesWildcards("cdn.images.example.com", "example.*"), true, "Assertion failed for fourth level subdomain");
  });

  it("should return true for regular match for punycode", function() {
    assert.strictEqual(matchesWildcards("xn--h1alffa9f.xn--p1ai", "xn--h1alffa9f.*"), true);
  });

  it("should return false for different second-level domain", function() {
    assert.strictEqual(matchesWildcards("bar.com", "foo.*"), false);
  });

  it("should return false for prefixed domain match", function() {
    assert.strictEqual(matchesWildcards("notbar.com", "bar.*"), false);
    assert.strictEqual(matchesWildcards("notcdn.bar.com", "cdn.bar.*"), false);
    assert.strictEqual(matchesWildcards("cdn.notbar.com", "bar.*"), false);
  });

  it("should return false for regular match for SLD", function() {
    assert.strictEqual(matchesWildcards("example.br.com", "example.*"), false);
  });

  it("should return false for regular match for domain in private registry in PSL", function() {
    assert.strictEqual(matchesWildcards("example.blogspot.com", "example.*"), false);
  });

  it("should return false for regular match for domain not in PSL", function() {
    assert.strictEqual(matchesWildcards("example.evil.biz", "example.*"), false, "Assertion failed for evil.biz");
    assert.strictEqual(matchesWildcards("example.evil", "example.*"), false, "Assertion failed for evil");
  });

  it("should return false for partial match of the domain", function() {
    assert.strictEqual(matchesWildcards("images.notexample.com", "example.*"), false);
  });

  it("should return false for regular match for domain not punycoded", function() {
    // left-hand side of elemhide filters, must be punycoded
    assert.strictEqual(matchesWildcards("xn--h1alffa9f.xn--p1ai", "россия.*"), false);
  });

  it("should return true for wildcards with port", function() {
    assert.strictEqual(matchesWildcards("abptestpages.org:5001", "abptestpages.*"), true);
  });
});
