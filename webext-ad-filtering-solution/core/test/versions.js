...
const {compareVersions} = require(LIB_FOLDER + "/versions.js");

describe("compareVersions()", function() {
  function checkEqual(v1, v2) {
    assert.strictEqual(compareVersions(v1, v2), 0, `${v1} and ${v2} should be equal`);
  }

  function checkLessThan(v1, v2) {
    assert.strictEqual(compareVersions(v1, v2), -1, `${v1} should be less than ${v2}`);
  }

  function checkGreaterThan(v1, v2) {
    assert.strictEqual(compareVersions(v1, v2), 1, `${v1} should be greater than ${v2}`);
  }
