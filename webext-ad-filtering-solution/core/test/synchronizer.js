...
describe("Synchronizer", function() {
  ...
  beforeEach(function() {
    ...
    // Sandbox required because the synchronizer does lots of stuff we want to
    // mock in unit tests.
    ...
    (
      ...
      {Subscription, CountableSubscription, FullUpdatableSubscription} =
  ...
  describe("It starts the synchronizer", function() {
    ...
    describe("Manifest V3 subscription", function() {
      ...
      it("Sends subscription metadata for enabled subscription", function() {
        let subscription = Subscription.fromURL("https://easylist-downloads.adblockplus.org/easylist.txt");
        filterStorage.addSubscription(subscription);
      ...
      it("Emits event on custom subscription update", async function() {
        let subscription = Subscription.fromURL("https://example.com/subscription");
        filterStorage.addSubscription(subscription);
        ...
        let notified = false;
        filterNotifier.on("subscription.fullUpdateReceived", sub => {
          notified = sub === subscription;
        });

        await runner.runScheduledTasks(12);
        assert.equal(notified, true);
      });

      it("Emits event on custom subscription on diff update", async function() {
        ...
        runner.registerHandler("/v3/diff/abpindo/187271f7a9a62cda9661034000d12367bdfbb78c.json", () => {
          return [200, EMPTY_DIFF_UPDATE];
        });

        let notified = false;
        filterNotifier.on("subscription.diffReceived", sub => {
          notified = sub === subscription;
        });
    ...
    it("gives the right error message when diff is outdated", async function() {
      Subscription.dnr = true;
      let testDiffSubscriptionUrl = "https://example.com/subscription";
      let invalidDiffUrl = "https://easylist-downloads.adblockplus.org/v3/diff/easylist/invalid.json";
      let subscription = Subscription.fromURL(testDiffSubscriptionUrl);
      subscription.diffURL = invalidDiffUrl;
      subscription.lastModified = "Last modified: 14 March 2023 00:06 UTC";
      filterStorage.addSubscription(subscription);

      return runner.runScheduledTasks(1).then(() => {
        assert.equal(subscription.downloadStatus, "synchronize_diff_too_old");
      });
    });
