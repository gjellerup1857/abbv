...
const {contentTypes} = require(LIB_FOLDER + "/contentTypes");
const {Filter} = require(LIB_FOLDER + "/filterClasses");
const {CombinedMatcher, newDefaultMatcher, Matcher} = require(LIB_FOLDER + "/matcher");
const {parseURL} = require(LIB_FOLDER + "/url");
...
describe("Matcher", function() {
  ...
  function checkSearch(filters, location, requestContentTypes, docDomain,
    ...
    for (let arg of [url, location]) {
      let result = matcher.search(arg, contentTypeMask, docDomain, sitekey, specificOnly, filterType);
  ...
  it("Filter search", function() {
    ...
    // Matching CSP filter
    let cspFilter = "$csp=script-src  https://abptestpages.org/lib/utils.js,domain=testpages.adblockplus.org";
    checkSearch(
      [cspFilter],
      "https://abptestpages.org/filters/csp_specific",
      ["CSP", "OTHER"],
    ...
    checkSearch(filters, "http://example.com/foos", "INVALID", "example.com", null, false, "all", {blocking: [], allowing: []});
