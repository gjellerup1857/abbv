...
describe("Filter storage", function() {
  ...
  beforeEach(async function() {
    // Sandbox is required here to get the stub io module and because
    // filterNotifier is a global singleton..
  ...
  it("Removing subscriptions", function() {
    ...
    assert.equal(Subscription.fromURL(subscription1.url), subscription1, "Subscription known before addition");
    ...
    compareSubscriptionList("Initial state", [subscription1, subscription2], [subscription1, subscription2]);
    assert.deepEqual(changes, [], "Received changes");

    assert.equal(Subscription.fromURL(subscription1.url), subscription1, "Subscription known after addition");

    changes = [];
    filterStorage.removeSubscription(subscription1);
    compareSubscriptionList("Removing first subscription", [subscription2], [subscription2]);
    ...
    assert.notEqual(Subscription.fromURL(subscription1.url), subscription1, "Subscription forgotten upon removal");
    Subscription.knownSubscriptions.delete(subscription1.url);

    changes = [];
    filterStorage.removeSubscription(subscription1);
    compareSubscriptionList("Removing already removed subscription", [subscription2], [subscription2]);
    assert.deepEqual(changes, [], "Received changes");
  ...
  it("Clearing", async function() {
    let subscription1 = Subscription.fromURL("https://test1/");
    let subscription2 = Subscription.fromURL("https://test2/");
    let filter1 = Filter.fromText("block_me");
    ...
    let subscription3 = await filterStorage.addFilter(filter1);
    assert.equal(filterStorage.getGroupForFilter(filter1), subscription3);
    assert.equal(Subscription.fromURL(subscription1.url), subscription1, "Subscription known after addition");

    filterStorage._clear();

    compareSubscriptionList("Clearing subscriptions", [], []);
    assert.equal(filterStorage.hasSubscription(subscription1), false);
    assert.equal(filterStorage.hasSubscription(subscription2), false);
    assert.equal(filterStorage.hasSubscription(subscription3), false);
    assert.deepEqual(Array.from(filterStorage.subscriptions()), []);
    assert.equal(filterStorage.getGroupForFilter(filter1), null);

    filterStorage.addSubscription(subscription1);
    compareSubscriptionList("Add", [subscription1], []);
    assert.equal(filterStorage.hasSubscription(subscription1), true);
  });
  ...
  it("Allows adding filters with metadata", async function() {
    ...
    assert.deepEqual(await filterStorage.getMetadataForFilter("another.example.com"), null);
    assert.deepEqual(await filterStorage.getMetadataForFilter("example.com$image"), null);
    await assert.rejects(filterStorage.setMetadataForFilter("example.com$image", {meta: "bar"}), "FilterStorageError");
    ...
    filterStorage.removeSubscription(subscription2);
    assert.deepEqual(await filterStorage.getMetadataForFilter("bar1"), null);
    assert.deepEqual(await filterStorage.getMetadataForFilter("bar2"), null);
    assert.deepEqual(await filterStorage.getMetadataForFilter("bar3"), null);
  });

  it("skips adding duplicate filters", async function() {
    let commentFilter = Filter.fromText("!comment");
    let metadata = {meta: "foo"};
    let subscription = await filterStorage.addFiltersWithMetadata(
      commentFilter, metadata
    );
    await filterStorage.addFiltersWithMetadata(commentFilter, metadata);
    assert.equal(subscription.filterCount, 1);
    compareSubscriptionList("Check for duplicates",
                            [subscription],
                            [subscription]);
  });
  ...
  it("Hit counts", async function() {
    ...
    await filterStorage.addFilter(filter1);
