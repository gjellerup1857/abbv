...
const callback = (environment, ...filters) => {
  ...
  const invokes = bound(globalThis);
  const classes = isExtensionContext$2 ? globalThis : secure(globalThis);
  const {Map: Map$9, RegExp: RegExp$3, Set: Set$2, WeakMap: WeakMap$5, WeakSet: WeakSet$3} = classes;
  ...
  const {Map: Map$8, WeakMap: WeakMap$4, WeakSet: WeakSet$2, setTimeout} = env;
  ...
  let {Math: Math$1, RegExp: RegExp$2} = $(window);
  ...
  function wrapPropertyAccess(object, property, descriptor,
    ...
    if (!propertyAccessors.has(object))
      propertyAccessors.set(object, new Map$6());
    ...
    let toBeWrapped = new Map$6([[property, descriptor]]);
  ...
  const {Error: Error$4, JSON: JSON$2, Map: Map$4, Response: Response$2, Object: Object$4} = $(window);
  ...
  let {Error: Error$3, JSON: JSON$1, Map: Map$3, Object: Object$3, Response: Response$1} = $(window);
  ...
  let {fetch: fetch$1, Map: Map$1, Object: Object$1, RegExp: RegExp$1, Response} = $(window);
  ...
  function replaceFetchResponse(search, replacement = "", needle = null) {
    ...
    if (!fetchRules) {
      ...
      window.fetch = proxy(fetch$1, (...args) => {
        const promise = apply$2(fetch$1, self, args).then(origResponse => {
