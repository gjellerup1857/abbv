...
const FORMAT_LIMIT_NUM_VALS=20;const FORMAT_NUM_FIRST_LAST_VALS=3;const FORMAT_NUM_SIG_DIGITS=7;function computeMaxSizePerColumn(e,t,n,r){const s=sizeFromShape(t);const a=r[r.length-1];const o=new Array(a).fill(0);const i=t.length;const c=n==="complex64"?createComplexTuples(e):e;if(i>1){for(let e=0;e<s/a;e++){const t=e*a;for(let e=0;e<a;e++){o[e]=Math.max(o[e],valToString(c[t+e],0,n).length)}}}return o}function subTensorToString(e,t,n,r,s,a=true){const o=n==="complex64"?2:1;const i=t[0];const c=t.length;if(c===0){if(n==="complex64"){const t=createComplexTuples(e);return[valToString(t[0],0,n)]}if(n==="bool"){return[boolNumToString(e[0])]}return[e[0].toString()]}if(c===1){if(i>FORMAT_LIMIT_NUM_VALS){const t=FORMAT_NUM_FIRST_LAST_VALS*o;let r=Array.from(e.slice(0,t));let a=Array.from(e.slice((i-FORMAT_NUM_FIRST_LAST_VALS)*o,i*o));if(n==="complex64"){r=createComplexTuples(r);a=createComplexTuples(a)}return["["+r.map(((e,t)=>valToString(e,s[t],n))).join(", ")+", ..., "+a.map(((e,t)=>valToString(e,s[i-FORMAT_NUM_FIRST_LAST_VALS+t],n))).join(", ")+"]"]}const t=n==="complex64"?createComplexTuples(e):Array.from(e);return["["+t.map(((e,t)=>valToString(e,s[t],n))).join(", ")+"]"]}const u=t.slice(1);const l=r.slice(1);const p=r[0]*o;const d=[];if(i>FORMAT_LIMIT_NUM_VALS){for(let t=0;t<FORMAT_NUM_FIRST_LAST_VALS;t++){const r=t*p;const a=r+p;d.push(...subTensorToString(e.slice(r,a),u,n,l,s,false))}d.push("...");for(let t=i-FORMAT_NUM_FIRST_LAST_VALS;t<i;t++){const r=t*p;const a=r+p;d.push(...subTensorToString(e.slice(r,a),u,n,l,s,t===i-1))}}else{for(let t=0;t<i;t++){const r=t*p;const a=r+p;d.push(...subTensorToString(e.slice(r,a),u,n,l,s,t===i-1))}}const h=c===2?",":"";d[0]="["+d[0]+h;for(let e=1;e<d.length-1;e++){d[e]=" "+d[e]+h}let m=",\n";for(let e=2;e<c;e++){m+="\n"}d[d.length-1]=" "+d[d.length-1]+"]"+(a?"":m);return d}function createComplexTuples(e){const t=[];for(let n=0;n<e.length;n+=2){t.push([e[n],e[n+1]])}return t}
...
function xAs4D(e){let t;if(e.rank===0||e.rank===1){t=reshape$1(e,[1,1,1,e.size])}else if(e.rank===2){t=reshape$1(e,[1,1,e.shape[0],e.shape[1]])}else if(e.rank===3){t=reshape$1(e,[1,e.shape[0],e.shape[1],e.shape[2]])}else{t=e}return t}
const batchNorm2d=op({batchNorm2d_:()=>{}});const batchNorm3d=op({batchNorm3d_:()=>{}});const batchNorm4d=op({batchNorm4d_:()=>{}});
...
const concat1d=op({concat1d_:()=>{}});const concat2d=op({concat2d_:()=>{}});const concat3d=op({concat3d_:()=>{}});const concat4d=op({concat4d_:()=>{}});
...
const INT32_MAX=2147483648;const searchSorted=op({searchSorted_:()=>{}});
...
function pad_(e,t,n=0){const r=convertToTensor(e,"x","pad");if(r.rank===0){throw new Error("pad(scalar) is not defined. Pass non-scalar to pad")}const s={paddings:t,constantValue:n};const a={x:r};return ENGINE.runKernel(PadV2,a,s)}const pad=op({pad_:pad_});const pad1d=op({pad1d_:()=>{}});const pad2d=op({pad2d_:()=>{}});const pad3d=op({pad3d_:()=>{}});const pad4d=op({pad4d_:()=>{}});
...
class MPRandGauss{constructor(e,t,n,r,s){this.mean=e;this.stdDev=t;this.dtype=n;this.nextVal=NaN;this.truncated=r;if(this.truncated){this.upper=this.mean+this.stdDev*2;this.lower=this.mean-this.stdDev*2}const a=s?s:Math.random();this.random=seedrandom.alea(a.toString())}nextValue(){if(!isNaN(this.nextVal)){const e=this.nextVal;this.nextVal=NaN;return e}let e,t;let n=false;while(!n){let r,s,a;do{r=2*this.random()-1;s=2*this.random()-1;a=r*r+s*s}while(a>=1||a===0);const o=Math.sqrt(-2*Math.log(a)/a);e=this.mean+this.stdDev*r*o;t=this.mean+this.stdDev*s*o;if(!this.truncated||this.isValidTruncated(e)){n=true}}if(!this.truncated||this.isValidTruncated(t)){this.nextVal=this.convertValue(t)}return this.convertValue(e)}convertValue(e){if(this.dtype==null||this.dtype==="float32"){return e}return Math.round(e)}isValidTruncated(e){return e<=this.upper&&e>=this.lower}}class RandGamma{constructor(e,t,n,r){this.alpha=e;this.beta=1/t;this.dtype=n;const s=r?r:Math.random();this.randu=seedrandom.alea(s.toString());this.randn=new MPRandGauss(0,1,n,false,this.randu());if(e<1){this.d=e+2/3}else{this.d=e-1/3}this.c=1/Math.sqrt(9*this.d)}nextValue(){let e,t,n,r,s,a;while(true){do{r=this.randn.nextValue();a=1+this.c*r}while(a<=0);a*=a*a;e=r*r;t=1-.331*e*e;n=.5*e+this.d*(1-a+Math.log(a));s=this.randu();if(s<t||Math.log(s)<n){break}}a=1/this.beta*this.d*a;if(this.alpha<1){a*=Math.pow(this.randu(),1/this.alpha)}return this.convertValue(a)}convertValue(e){if(this.dtype==="float32"){return e}return Math.round(e)}}class UniformRandom{constructor(e=0,t=1,n,r){this.canReturnFloat=()=>this.dtype==null||this.dtype==="float32";this.min=e;this.range=t-e;this.dtype=n;if(r==null){r=Math.random()}if(typeof r==="number"){r=r.toString()}if(!this.canReturnFloat()&&this.range<=1){throw new Error(`The difference between ${e} - ${t} <= 1 and dtype is not float`)}this.random=seedrandom.alea(r)}convertValue(e){if(this.canReturnFloat()){return e}return Math.round(e)}nextValue(){return this.convertValue(this.min+this.range*this.random())}}
...
function getNoiseShape(e,t){if(t==null){return e.shape.slice()}if(arraysEqual(e.shape,t)){return t}if(e.shape.length===t.length){const n=[];for(let r=0;r<e.shape.length;r++){if(t[r]==null&&e.shape[r]!=null){n.push(e.shape[r])}else{n.push(t[r])}}return n}return t}
...
function binarySearch(e,t,n){return binarySearch_(e,t,n||defaultComparator)}function binarySearch_(e,t,n){let r=0;let s=e.length;let a=0;let o=false;while(r<s){a=r+(s-r>>>1);const i=n(t,e[a]);if(i>0){r=a+1}else{s=a;o=!i}}return o?r:-r-1}
function intersectionOverUnion(e,t,n){const r=e.subarray(t*4,t*4+4);const s=e.subarray(n*4,n*4+4);const a=Math.min(r[0],r[2]);const o=Math.min(r[1],r[3]);const i=Math.max(r[0],r[2]);const c=Math.max(r[1],r[3]);const u=Math.min(s[0],s[2]);const l=Math.min(s[1],s[3]);const p=Math.max(s[0],s[2]);const d=Math.max(s[1],s[3]);const h=(i-a)*(c-o);const m=(p-u)*(d-l);if(h<=0||m<=0){return 0}const f=Math.max(a,u);const g=Math.max(o,l);const y=Math.min(i,p);const x=Math.min(c,d);const T=Math.max(y-f,0)*Math.max(x-g,0);return T/(h+m-T)}
var Reduction;(function(e){e[e["NONE"]=0]="NONE";e[e["MEAN"]=1]="MEAN";e[e["SUM"]=2]="SUM";e[e["SUM_BY_NONZERO_WEIGHTS"]=3]="SUM_BY_NONZERO_WEIGHTS"})(Reduction||(Reduction={}));const computeWeightedLoss=op({computeWeightedLoss_:()=>{}});
const cosineDistance=op({cosineDistance_:()=>{}});const hingeLoss=op({hingeLoss_:()=>{}});
function sigmoidCrossEntropyWithLogits_(e,t){const n=convertToTensor(e,"labels","sigmoidCrossEntropyWithLogits");const r=convertToTensor(t,"logits","sigmoidCrossEntropyWithLogits");assertShapesMatch(n.shape,r.shape,"Error in sigmoidCrossEntropyWithLogits: ");const s=relu$1(r);const a=mul(r,n);const o=log1p(exp(neg(abs(r))));return add(sub(s,a),o)}const sigmoidCrossEntropy=op({sigmoidCrossEntropy_:()=>{}});
function softmaxCrossEntropyWithLogits_(e,t,n=-1){if(n===-1){n=t.rank-1}if(n!==t.rank-1){throw Error(`Softmax cross entropy along a non-last dimension is not yet `+`supported. Labels / logits was rank ${t.rank} `+`and dim was ${n}`)}const r=customGrad(((e,t,r)=>{const s=true;const a=logSumExp(t,[n],s);const o=sub(cast$1(t,"float32"),a);r([e,o]);const i=neg(mul(o,e));const c=sum$1(i,[n]);const u=(e,t)=>{const[r,s]=t;const a=expandShapeToKeepDim(e.shape,[n]);return[mul(reshape$1(e,a),sub(cast$1(r,"float32"),exp(s))),mul(reshape$1(e,a),sub(exp(s),cast$1(r,"float32")))]};return{value:c,gradFunc:u}}));return r(e,t)}const softmaxCrossEntropy=op({softmaxCrossEntropy_:()=>{}});
...
const delayCallback=(()=>{if(typeof requestAnimationFrame!=="undefined"){return requestAnimationFrame}else if(typeof setImmediate!=="undefined"){return setImmediate}return e=>e()})();
...
const ARROW="->";const ARROW_REGEX=/->/g;const COMMA=",";const ELLIPSIS="...";function findTermsWithDim(e,t){const n=[];for(let r=0;r<e.length;++r){if(e[r].length===0||e[r].indexOf(t)!==-1||t===-1){n.push(r)}}return n}
...
class StringNGramsOp{constructor(e,t,n,r,s,a){this.separator=encodeString(e);this.nGramWidths=t;this.leftPad=encodeString(n);this.rightPad=encodeString(r);this.padWidth=s;this.preserveShort=a}getPadWidth(e){return Math.min(this.padWidth<0?e-1:this.padWidth,e-1)}getNumNGrams(e,t){const n=this.getPadWidth(t);return Math.max(0,e+2*n-t+1)}createNGrams(e,t,n,r,s,a){for(let o=0;o<s;++o){const i=this.getPadWidth(a);const c=Math.max(0,i-o);const u=Math.max(0,i-(s-(o+1)));const l=a-(c+u);const p=t+(c>0?0:o-i);let d=0;d+=c*this.leftPad.length;for(let t=0;t<l;++t){d+=e[p+t].length}d+=u*this.rightPad.length;const h=c+u+l-1;d+=h*this.separator.length;n[r+o]=new Uint8Array(d);const m=n[r+o];let f=0;const g=e=>e.forEach((e=>m[f++]=e));for(let e=0;e<c;++e){g(this.leftPad);g(this.separator)}for(let t=0;t<l-1;++t){g(e[p+t]);g(this.separator)}if(l>0){g(e[p+l-1]);for(let e=0;e<u;++e){g(this.separator);g(this.rightPad)}}else{for(let e=0;e<u-1;++e){g(this.rightPad);g(this.separator)}g(this.rightPad)}}}compute(e,t){const n=e.length;const r=t.length;if(r>0){let e=t[0];if(e!==0){throw new Error(`First split value must be 0, got ${e}`)}for(let s=1;s<r;++s){let r=t[s]>=e;r=r&&t[s]<=n;if(!r){throw new Error(`Invalid split value ${t[s]}, must be in [${e}, ${n}]`)}e=t[s]}if(e!==n){throw new Error(`Last split value must be data size. Expected ${n}, got ${e}`)}}const s=r-1;const a=getArrayFromDType("int32",r);if(n===0||r===0){const e=new Array(n);for(let e=0;e<=s;++e){a[e]=0}return[e,a]}a[0]=0;for(let e=1;e<=s;++e){const n=t[e]-t[e-1];let r=0;this.nGramWidths.forEach((e=>{r+=this.getNumNGrams(n,e)}));if(this.preserveShort&&n>0&&r===0){r=1}a[e]=a[e-1]+r}const o=new Array(a[s]);for(let n=0;n<s;++n){const r=t[n];let s=a[n];this.nGramWidths.forEach((a=>{const i=t[n+1]-t[n];const c=this.getNumNGrams(i,a);this.createNGrams(e,r,o,s,c,a);s+=c}));if(this.preserveShort&&s===a[n]){const a=t[n+1]-t[n];if(a===0){continue}const i=a+2*this.padWidth;const c=1;this.createNGrams(e,r,o,s,c,i)}}return[o,a]}}
const subImpl=createSimpleBinaryKernelImpl(((e,t)=>e-t));
const comparePair=(e,t)=>{const n=t.value-e.value;return n===0?e.index-t.index:n};function select$1(e,t,n=0,r=e.length-1){while(r>n){if(r-n>600){const s=r-n+1;const a=t-n+1;const o=Math.log(s);const i=.5*Math.exp(2*o/3);const c=.5*Math.sqrt(o*i*(s-i)/s)*Math.sign(a-s/2);const u=Math.max(n,Math.floor(t-a*i/s+c));const l=Math.min(r,Math.floor(t+(s-a)*i/s+c));select$1(e,t,u,l)}const s=e[t];let a=n;let o=r;swap(e,n,t);if(comparePair(e[r],s)>0){swap(e,n,r)}while(a<o){swap(e,a,o);a++;o--;while(comparePair(e[a],s)<0){a=a+1}while(comparePair(e[o],s)>0){o=o-1}}if(comparePair(e[n],s)===0){swap(e,n,o)}else{o=o+1;swap(e,o,r)}if(o<=t){n=o+1}if(t<=o){r=o-1}}}
