...
/**
 * Adds a filter to the user's custom filters.
 *
...
export async function addFilter(filterText, origin, expiresAt, autoExtendMs) {
  ...
  if (autoExtendMs) metadata.autoExtendMs = autoExtendMs;
...
function parseFilter(text) {
  ...
  if (filterText) {
    ...
    if (filterText[0] == "[") {
      ...
      if (filterError) error = toSerializableFilterError(filterError);
    }
  }
