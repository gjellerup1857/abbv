...
describe("test advanced tab editing custom filters", function () {
  ...
  it("should disable/enable a custom filter", async function () {
    ...
    expect(
      await advancedPage.isCustomFilterListsFirstItemToggleSelected("true")
    ).to.be.true;
    await advancedPage.clickCustomFilterListsFirstItemToggle();
  ...
  it("should display copy and delete buttons", async function () {
    ...
    expect(await advancedPage.isCopyCustomFLButtonDisplayed()).to.be.true;
    expect(await advancedPage.isDeleteCustomFLButtonDisplayed()).to.be.true;
    await advancedPage.clickDeleteCustomFLButton();
  });
  ...
  it("should sort custom filters", async function () {
    ...
    expect(await advancedPage.isCustomFilterListsNthItemCheckboxChecked("1")).to
      .be.true;
    expect(await advancedPage.isCustomFilterListsNthItemCheckboxChecked("2")).to
      .be.true;
    expect(await advancedPage.isCustomFilterListsNthItemCheckboxChecked("3")).to
      .be.true;
    expect(await advancedPage.isCustomFilterListsNthItemCheckboxChecked("4")).to
      .be.true;
    await advancedPage.clickCustomFLTableHeadCheckbox();
    expect(
      await advancedPage.isCustomFilterListsNthItemCheckboxChecked("1", true)
    ).to.be.true;
    expect(
      await advancedPage.isCustomFilterListsNthItemCheckboxChecked("2", true)
    ).to.be.true;
    expect(
      await advancedPage.isCustomFilterListsNthItemCheckboxChecked("3", true)
    ).to.be.true;
    expect(
      await advancedPage.isCustomFilterListsNthItemCheckboxChecked("4", true)
    ).to.be.true;
  ...
  it("should edit a custom filter", async function () {
    ...
    expect(await advancedPage.verifyTextPresentInCustomFLTable(inputText, 200))
      .to.be.false;
  });
  ...
  it("should edit a custom filter into an erroneous filter", async function () {
    ...
    await browser.keys("Enter");
    expect(await advancedPage.isCustomFilterListsFirstItemErrorIconDisplayed())
      .to.be.true;
    await browser.refresh();
    await switchToABPOptionsTab();
    await advancedPage.init();
    expect(await advancedPage.verifyTextPresentInCustomFLTable(inputText)).to.be
      .true;
    expect(await advancedPage.verifyTextPresentInCustomFLTable(",##foo", 200))
      .to.be.false;
    expect(
      await advancedPage.isCustomFilterListsFirstItemAlertIconDisplayed(true)
    ).to.be.true;
  });
  ...
  it("should search for custom filters", async function () {
    ...
    await advancedPage.typeTextToAddCustomFilterListInput("filter");
    expect(await advancedPage.isCustomFilterListsNthItemCheckboxChecked("1")).to
      .be.true;
    expect(await advancedPage.isCustomFilterListsNthItemCheckboxChecked("2")).to
      .be.true;
  ...
  it("should copy custom filters", async function () {
    ...
    await browser.keys("Enter");
    try {
      expect(await advancedPage.verifyTextPresentInCustomFLTable(inputText)).to
        .be.true;
    } catch (Exception) {
      await browser.keys([pasteKey, "V"]);
      await browser.keys("Enter");
