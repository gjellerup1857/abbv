...
describe("test DC filterlist setting for premium users", function () {
  ...
  it("should disable/enable distraction control for premium user from the General tab", async function () {
    ...
    expect(await advancedPage.isPremiumDistractionControlFLDisplayed()).to.be
      .false;
    ...
    expect(await popupPage.isBlockMoreDistractionsToggleSelected()).to.be.false;
    ...
    expect(await advancedPage.isPremiumDistractionControlFLDisplayed()).to.be
      .true;
    expect(
      await advancedPage.isPremiumDistractionControlFLStatusToggleSelected()
    ).to.be.true;
    await popupPage.init(popupUrl, tabId);
    expect(await popupPage.isBlockMoreDistractionsToggleSelected()).to.be.true;
  });
