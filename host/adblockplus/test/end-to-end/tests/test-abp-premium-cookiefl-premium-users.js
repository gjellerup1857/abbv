...
describe("test cookie consent filterlist setting for premium users", function () {
  ...
  it("should enable/disable cookie filterlist for premium user from the Advanced page", async function () {
    ...
    expect(await advancedPage.isPremiumBlockCookieConsentPopupsFLDisplayed()).to
      .be.false;
    await advancedPage.clickAddBuiltinFilterListButton();
    await advancedPage.clickBuiltInFLTableItem(
      "Premium - Block cookie consent pop-ups"
    );
    expect(await advancedPage.isPremiumBlockCookieConsentPopupsFLDisplayed()).to
      .be.true;
    expect(
      await advancedPage.isPremiumBlockCookieConsentPopupsFLStatusToggleSelected()
    ).to.be.true;
  ...
  it("should enable/disable cookie filterlist for premium user from the General tab", async function () {
    ...
    expect(await generalPage.isBlockCookieConsentPopupsDialogDisplayed()).to.be
      .true;
  ...
  it("should enable/disable cookie filterlist for premium user from the Popup page", async function () {
    ...
    expect(await popupPage.isCookieConsentPopupsPopupDisplayed()).to.be.true;
    await popupPage.clickCookieConsentPopupsPopupNotNowButton();
    expect(await popupPage.isCookieConsentPopupsPopupDisplayed()).to.be.false;
    expect(await popupPage.isBlockCookieConsentPopupsToggleSelected()).to.be
      .false;
    await popupPage.clickBlockCookieConsentPopupsToggle();
