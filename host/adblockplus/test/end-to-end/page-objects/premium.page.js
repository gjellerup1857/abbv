...
class PremiumPage extends BasePage {
  ...
  async clickGetStartedWithABPPremiumButton() {
    await this.waitForEnabledThenClick(
      this.getStartedWithABPPremiumButton,
      20000
    );
  }

  async clickPaymentConfirmedLabel() {
    await this.waitForEnabledThenClick(this.paymentConfirmedLabel);
  }

  async clickPayWithCreditCardButton() {
    await this.waitForEnabledThenClick(this.payWithCreditCardButton);
  }

  async clickPremiumCheckoutButton() {
    await this.waitForEnabledThenClick(this.premiumCheckoutButton);
  }

  async getPaymentConfirmedLabelText() {
    await this.waitForDisplayedNoError(
      this.paymentConfirmedLabel,
      false,
      15000
    );
