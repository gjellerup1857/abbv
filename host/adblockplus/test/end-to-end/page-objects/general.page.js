...
class GeneralPage extends BasePage {
  ...
  get blockAdditionalTrackingDescription() {
    return $(
      "//li[@aria-label='Block additional tracking']/" +
  ...
  get blockCookieConsentPopupsDialogNoButton() {
    return $(
      "//*[@id='dialog-content-optIn-premium-subscription']/" +
  ...
  get deutschPlusEnglishListItem() {
    return $("//li[contains(@role, 'option') and text()='Deutsch + English']");
  }
  ...
  get blockPushNotificationsDescription() {
    return $(
      "//li[@aria-label='Block push notifications']/" +
        "div[@class='description-container']/p"
    );
  }
  ...
  get italianoPlusEnglishListItem() {
    return $("//li[contains(@role, 'option') and text()='italiano + English']");
  }
  ...
  async moreFilterListsTableItemByLabel(label) {
    return $(
      "//ul[@id='more-list-table']/li[@aria-label='" + label + "']/span"
    );
  }

  get nederlandsPlusEnglishListItem() {
    return $(
      "//li[contains(@role, 'option') and text()='Nederlands + English']"
    );
  }
  ...
  get notifyLanguageFilterListsTooltipIcon() {
    return $("//li[@data-pref='recommend_language_subscriptions']/io-popout");
  }
  ...
  async clickAllowAcceptableAdsCheckbox() {
    await this.waitForEnabledThenClick(this.allowAcceptableAdsCheckbox);
  }

  async clickBlockAdditionalTrackingCheckbox() {
    await this.waitForEnabledThenClick(this.blockAdditionalTrackingCheckbox);
  }

  async clickBlockCookieConsentPopupsDialogNoButton() {
    await this.waitForEnabledThenClick(
      this.blockCookieConsentPopupsDialogNoButton
    );
  }

  async clickBlockCookieConsentPopupsDialogOkButton() {
    await this.waitForEnabledThenClick(
      this.blockCookieConsentPopupsDialogOkButton
    );
  }

  async clickBlockCookieConsentPopupsCheckbox() {
    await this.waitForEnabledThenClick(this.blockCookieConsentPopupsCheckbox);
  }

  async clickBlockMoreDistractionsCheckbox() {
    await this.waitForEnabledThenClick(this.blockMoreDistractionsCheckbox);
  }
  ...
  async clickUpgradeButtonGeneral() {
    await this.waitForEnabledThenClick(this.upgradeButtonGeneral);
  }
  ...
  async getBlockSocialMediaIconsTrackingDescriptionText() {
    return await (
      await this.blockSocialMediaIconsTrackingDescription
    ).getText();
  }

  async getLanguagesTableEmptyPlaceholderText() {
    await (
      await this.languagesTableEmptyPlaceholder
  ...
  async getMoreFilterListsTableItemByLabelText(label) {
    return await (await this.moreFilterListsTableItemByLabel(label)).getText();
  }

  async getNotifyLanguageFilterListsTooltipText() {
    return await (await this.notifyLanguageFilterListsTooltipText).getText();
  }

  async getPredefinedDialogTitleText() {
    await (await this.predefinedDialogTitle).waitForEnabled({ timeout: 3000 });
  ...
  async isAllowAcceptableAdsCheckboxSelected(
    ...
    await (
      await this.allowAcceptableAdsCheckbox
    ).waitForEnabled({ timeout: 3000 });
    return await this.waitForSelectedNoError(
      this.allowAcceptableAdsCheckbox,
      reverse,
      timeoutMs
    );
  }
  ...
  async isBlockCookieConsentPopupsItemDisplayed(reverseOption = false) {
    return await this.waitForDisplayedNoError(
      this.blockCookieConsentPopupsItem,
      reverseOption
    );
  }
  ...
  async isBlockMoreDistractionsCheckboxSelected(reverse = false) {
    await (
      await this.blockMoreDistractionsCheckbox
    ).waitForEnabled({ timeout: 3000 });
    return await this.waitUntilAttributeValueIs(
      this.blockMoreDistractionsCheckbox,
      "aria-checked",
      "true",
      3000,
      reverse
  ...
  async isBlockMoreDistractionsItemDisplayed(reverseOption = false) {
    return await this.waitForDisplayedNoError(
      this.blockMoreDistractionsItem,
      reverseOption
    );
  }
  ...
  async isBlockPushNotificationsCheckboxSelected(reverse = false) {
    await (
      await this.blockPushNotificationsCheckbox
    ).waitForEnabled({ timeout: 3000 });
    return await this.waitUntilAttributeValueIs(
      this.blockPushNotificationsCheckbox,
      "aria-checked",
      "true",
      3000,
      reverse
  ...
  async isBlockSocialMediaIconsTrackingCheckboxSelected(reverse = false) {
    await (
      await this.blockSocialMediaIconsTrackingCheckbox
    ).waitForEnabled({ timeout: 3000 });
    return await this.waitUntilAttributeValueIs(
      this.blockSocialMediaIconsTrackingCheckbox,
      "aria-checked",
      "true",
      3000,
      reverse
  ...
  async isDeutschPlusEnglischLanguageTableItemDisplayed(reverseOption = false) {
    return await this.waitForDisplayedNoError(
      this.deutschPlusEnglischLanguageTableItem,
      reverseOption
    );
  }
  ...
  async isFilterListsSuggestionsCheckboxSelected(
    ...
    return await this.waitUntilAttributeValueIs(
      this.filterListsSuggestionsCheckbox,
      "aria-checked",
      "true",
      timeout,
      reverse
  ...
  async isItalianoPlusEnglishLanguageTableItemDisplayed(wait = false) {
    ...
    if (!wait) {
      returnValue = await (
        await this.italianoPlusEnglishLanguageTableItem
      ).isDisplayed();
    } else {
  ...
  async isListeFRPlusEasylistLanguageTableItemDisplayed() {
    return await (
      await this.listeFRPlusEasylistLanguageTableItem
    ).isDisplayed();
  }

  async isNotifyLanguageFilterListsCheckboxDisplayed(reverseOption = false) {
    return await this.waitForDisplayedNoError(
      this.notifyLanguageFilterListsCheckbox,
      reverseOption
    );
  }
  ...
  async isTrackingWarningNotDisplayed() {
    return await this.waitForDisplayedNoError(this.trackingWarning, true);
  }

  async isUpgradeButtonGeneralDisplayed(timeout) {
    return await this.waitForDisplayedNoError(
      this.upgradeButtonGeneral,
      false,
      timeout
    );
  }
  ...
  async switchToAAInfoTab() {
    await this.switchToTab(
      "https://adblockplus.org/acceptable-" +
