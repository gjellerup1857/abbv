...
const onPageClickIconStyleTextContent =
  " #onPageIcon { display: flex; } " +
...
const loadIconResources = function (base, callback) {
  ...
  function loadFont(name, style, weight, unicodeRange) {
    return new FontFace("Lato", `url(${browser.runtime.getURL(`/fonts/${name}.woff`)}`, {
      style,
      weight,
      unicodeRange,
    });
  ...
  fonts.push(
    new FontFace("AdBlock Icons", `url(${browser.runtime.getURL("/icons/adblock-icons.woff2")}`, {
      style: "normal",
      weight: "normal",
    }),
  );
...
const addOnPageIcon = function () {
  ...
  if (mainBody) {
    ...
    overlayElement.style.cssText =
      "display: none; padding: 0; margin: 0; position: fixed;  z-index: 2147483647 !important;";
    ...
    const noChannelsFoundMessageContent = overlayChildElement.querySelector(
    ...
    if (btnAllowAll) {
      btnAllowAll.onclick = function closedClicked(event) {
        ...
        browser.runtime.sendMessage({
          ...
          msg: "whitelist_all_youtube_clicked",
        });
        browser.runtime
          .sendMessage({ command: "allowAllSubscribedChannel", channelNames: channelNameObs })
          .then(() => {
    ...
    if (btnBlockAll) {
      btnBlockAll.onclick = function closedClicked(event) {
        ...
        browser.runtime.sendMessage({
          command: "recordGeneralMessage",
          msg: "remove_all_youtube_clicked",
        });
        browser.runtime
          .sendMessage({ command: "blockAllSubscribedChannel", channelNames: channelNameObs })
          .then(() => {
            // eslint-disable-next-line no-use-before-define
            addInPageIcons();
            processingContent.style.display = "none";
...
const configureChildElements = function (parentNodeArg, adsAllowedArg) {
  ...
  const lineOneClickHandler = function (event) {
    ...
    browser.runtime
      .sendMessage({
        command: "removeAllowlistFilterForYoutubeChannel",
        text: parentNode.dataset.filterText,
      })
      .then(() => {
        browser.runtime.sendMessage({ command: "getAllAdsAllowedUserFilters" }).then((response) => {
          ...
          browser.runtime.sendMessage({
            command: "recordGeneralMessage",
            msg: "remove_whitelist_youtube_clicked",
          });
        });
      });
  };
  ...
  browser.runtime.sendMessage({ command: "getAllAdsAllowedUserFilters" }).then((userFilters) => {
    ...
    if (initialAdd) {
      for (const [channelName] of Object.entries(channelNameObs)) {
        ...
        const notifcationElem = channelNameObs[channelName].node.querySelector(
...
const addUIEventListers = function (toContentScriptEventName) {
  ...
  document.addEventListener(toContentScriptEventName, (event) => {
    if (
      event &&
      event.detail &&
      event.detail.actionName === "yt-append-continuation-items-action"
    ) {
      removeInPageIcons();
      addInPageIcons(true);
    }
